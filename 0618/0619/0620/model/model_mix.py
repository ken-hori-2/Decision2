...
def perceptron(x1, w1):
	# w1 = -1 # 2
	y = x1*(-w1 + 4) # b = x1*(w1 + 4) # 3  w1 * x1 + b > 1  -> w1 * x1 + b -1 > 0
	if 1 < y: # b:
		return 0
	else:
		return 1
...
for x in range(X):
    ...
    for i in range(N):
        ...
        DISCOVERY_RATE[x][i] = COUNT_RATE[x] / (i + 1)
        CONTINUE_NUM[x][i] = COUNT[x]

        IGNITION_RESULT[x][i] = perceptron(DISCOVERY_RATE[x][i], CONTINUE_NUM[x][i])
