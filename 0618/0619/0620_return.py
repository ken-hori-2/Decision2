...
# meeting0620_2_return.py


N = 1  # エージェントの個数
SIZE = N   # 仮想空間のサイズ
TIMELIMIT = 5 #10  # シミュレーションの打ち切り時刻
SEED = 65535  # 乱数の初期化
# R = 15  # 感染範囲
...
for t in range(TIMELIMIT):
    ...
    # im += ax[3].bar(x[1], y1, color=['orange'], width = 0.5, label="一致") # , tick_label="Add Stress") #STRESS, color=['orange'], width = 0.5, label="Stress", tick_label="Add Stress")
    # im += ax[3].bar(x[1], y2, bottom = y1, color=['blue'], width = 0.5, label="不一致") # , tick_label="Add Stress")
    # # im += ax[3].pie(x2, labels=Label) # , color = CL)
    # # im += ax[3].axis('equal')
    # im += ax[3].bar(x[0], 0)
    # im += ax[3].bar(x[2], 0)
    im += ax[3].stackplot(T, MATCH, STRESS, colors=["orange","r"], alpha=0.7)
    print(STRESS)
