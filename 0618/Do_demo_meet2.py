...
N = 1  # エージェントの個数
SIZE = N   # 仮想空間のサイズ
TIMELIMIT = 10  # シミュレーションの打ち切り時刻
SEED = 65535  # 乱数の初期化
# R = 15  # 感染範囲
SPEED = N/50  # エージェントの歩幅
TREATMENT_PERIOD = 3 # 10  # 感染してから治るまでの期間
...
class Agent:
    ...
    def __init__(self, state):
        ...
        self.y = 0.0 #random.randint(1,SIZE)  # y座標の初期値
        radian = math.radians(random.randint(1,360))
        self.x_v = SPEED #0.01#math.cos(radian)*SPEED  # x方向の速さ
    ...
    def _update_xy(self):
        ...
        エージェントのxy座標を更新する
        """
        if self.x + self.x_v < 0:
            self.x = 0
            # self.x_v *= -1
            self.state = "S"
            
        if SIZE < self.x + self.x_v:
            # self.x_v *= 0 #-1
            self.x = 0 #self.x_v
    ...
    def _state_I(self):  # 状態Iの計算メソッド
        self.term += 1  # 感染日数を追加
        if self.term > TRIGAR : # 5 : # TREATMENT_PERIOD:  # 一定の感染日数が経つと免疫を獲得する
            self.state = "R"
...
def scatter_plot(image, n, xlistS, ylistS, xlistI, ylistI, xlistR, ylistR, xlistD, ylistD):
    ...
    image += ax[n].plot(xlistI, ylistI, ">", markersize=12, label="Infected", color="r") #状態Iのプロット
...
for t in range(TIMELIMIT):
    ...
    im.append(ax[0].text(0.4, 0.55, time_str))
    ims.append(im)
